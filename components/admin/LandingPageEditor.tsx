...
import { Plus, GripVertical, Trash2, X, Text, Table, AlignLeft, List } from 'lucide-react'
...
export function LandingPageEditor({ initialData = null }: { initialData?: any }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-black/5 overflow-hidden flex flex-col md:flex-row min-h-[60vh]">
      ...
      {/* Editor Pane */}
      <div className="flex-1 p-6 md:p-10 max-h-[75vh] overflow-y-auto">
        {activeTab === 'basic' && (
          <div className="flex flex-col gap-6 max-w-xl">
             ...
             <label className="flex flex-col gap-2 text-sm text-text-muted">
                Document Title
                <input type="text" className="p-3 border rounded-xl" defaultValue={initialData?.title} />
             </label>
             <label className="flex flex-col gap-2 text-sm text-text-muted">
                URL Slug
                <input type="text" className="p-3 border rounded-xl font-mono" defaultValue={initialData?.slug} />
             </label>
             <label className="flex flex-col gap-2 text-sm text-text-muted">
                SEO Description
                <textarea className="p-3 border rounded-xl" rows={3} defaultValue={initialData?.meta_description} />
             </label>
          </div>
        )}
        ...
        {activeTab === 'hero' && (
          <div className="flex flex-col gap-6 max-w-xl">
             ...
             <label className="flex flex-col gap-2 text-sm text-text-muted">
                Hero Headline
                <input type="text" className="p-3 border rounded-xl" defaultValue={initialData?.hero_headline} />
             </label>
             <label className="flex flex-col gap-2 text-sm text-text-muted">
                Hero Subtext
                <textarea className="p-3 border rounded-xl" rows={3} defaultValue={initialData?.hero_subtext} />
             </label>
             <label className="flex flex-col gap-2 text-sm text-text-muted">
                Badge Text
                <input type="text" className="p-3 border rounded-xl" defaultValue={initialData?.hero_badge} />
             </label>
          ...
          <div className="flex flex-col gap-6 max-w-2xl">
            <div className="flex justify-between items-center border-b pb-2">
              ...
              <div className="flex gap-2">
                <button onClick={() => addBlock('text')} className="px-3 py-1 bg-cream rounded hover:bg-cream-dark text-xs"><Text size={14} className="inline mr-1" /> Text</button>
                <button onClick={() => addBlock('specs_table')} className="px-3 py-1 bg-cream rounded hover:bg-cream-dark text-xs"><Table size={14} className="inline mr-1" /> Table</button>
              </div>
            </div>
        ...
        {activeTab === 'settings' && (
          <div className="flex flex-col gap-6 max-w-xl">
             ...
             <label className="flex items-center gap-3 cursor-pointer p-4 bg-cream rounded-xl">
               <input type="checkbox" className="w-5 h-5 rounded text-forest focus:ring-forest" defaultChecked={initialData?.show_chatbot ?? true} />
               <span className="text-sm font-medium">Enable Contextual AI Chatbot</span>
             </label>
             <label className="flex items-center gap-3 cursor-pointer p-4 bg-cream rounded-xl">
               <input type="checkbox" className="w-5 h-5 rounded text-forest focus:ring-forest" defaultChecked={initialData?.show_inquiry_form ?? true} />
               <span className="text-sm font-medium">Show Inquiry Form</span>
             </label>
             <label className="flex items-center gap-3 cursor-pointer p-4 bg-cream rounded-xl">
               <input type="checkbox" className="w-5 h-5 rounded text-forest focus:ring-forest" defaultChecked={initialData?.is_published ?? false} />
