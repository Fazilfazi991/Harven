...
export default function BrandsCMS() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setCurrentBrand({...currentBrand, image_url: data.publicUrl})
    } catch (err) {
      console.error(err)
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={5} className="py-20 text-center">
                    ...
                    <div className="text-text-dark font-medium">No Brands Found</div>
                    <div className="text-text-muted text-xs mt-1">Start by adding your first signature brand item.</div>
                  </td>
                </tr>
              ) : brands.map((row) => (
                <tr key={row.id} className="border-b border-black/5 hover:bg-cream/30 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <div className="text-[0.75rem] text-text-muted line-clamp-2 max-w-xs">{row.origin} &bull; {row.packaging} &bull; MOQ: {row.moq}</div>
                  </td>
                  <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-3xl rounded-[32px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300 flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSave} className="p-8 space-y-6 overflow-y-auto flex-1">
              ...
              <div>
                <label className="block text-xs font-bold text-text-muted uppercase tracking-wider mb-2">Image</label>
                <div className="flex gap-4 items-start">
                  ...
                  <div className="relative">
                    <input 
                      ...
                      disabled={uploading}
                    />
                    <button type="button" className="px-4 py-3 bg-cream-warm border border-cream-dark rounded-xl flex items-center gap-2 text-sm font-medium hover:bg-cream-dark transition-colors">
                ...
                {currentBrand.image_url && (
                  <div className="mt-4 border border-black/5 rounded-xl overflow-hidden w-24 h-24 bg-cream">
                     <img src={currentBrand.image_url} alt="Preview" className="w-full h-full object-cover" />
                  </div>
                )}
              </div>
